...
function BuyModal({ closeModal, onConfirmClick, loading = false }) {
  ...
  function handleConfirmClick() {
    const val = {
      ...
      receive_token_decimals: selectedToken?.number_of_decimals,
      receive_token_image: selectedToken?.token_image,
      offer_token: selectedColateralToken?.token_address,
      offer_token_symbol: selectedColateralToken?.symbol,
      offer_token_name: selectedColateralToken?.name,
      offer_token_decimals: selectedColateralToken?.number_of_decimals,
      offer_token_image: selectedColateralToken?.token_image,
      trader_wallet_address: account?.address,
      is_broker: false,
      broker_fee: 0,
    };
    onConfirmClick(val, selectedToken, selectedColateralToken);
  }
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center justify-between px-[14px] mt-[10px] md:mt-4 text-sm font-medium text-gray500 dark:text-gray500Dark bg-gray100 dark:bg-gray100Dark border border-gray200 dark:border-gray200Dark rounded-[14px] p-[10px] md:p-[14px]">
                <p className="flex flex-col gap-y-1">
                  Token Price
                  <span className="text-base font-semibold text-baseWhiteDark dark:text-baseWhite">
